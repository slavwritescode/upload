...
const processData = async (
    ...
    try {
        ...
        if (download && providerName !== undefined) {
            ...
            if (normalizedProvider !== '') {
                ...
                deliveredFromProvider = (providerStatuses['s1'] || 0) + (providerStatuses['s4'] || 0) + (providerStatuses['s5'] || 0);
            }

            const limitForProvider = Constants['eventTypes'][eventType]['providerLimit'];
            if (deliveredFromProvider >= limitForProvider) throw new Error('Provider limit reached: ' + messageId + ' (' + locale + '/' + providerName + ')');
        }
        ...
        if (dataType === 'textMessages') {
            ...
            if (download) {
                ...
                // console.log('Temp solution...'); // the type has to be dynamic !!!
                file = new File([blob], assetBaseName, { type: 'application/pdf' });
